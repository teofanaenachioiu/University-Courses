...
int find(Repo* memory, int id)
{
	...
	*/
	...
	{
		if (getId(getProdus(getAll(memory), i)) == id)
		{
			return i;
		}
	}
...
int updatePret(Repo* memory, Produs* prod, double pret)
{
	...
	*/
	...
	if (poz != -1)
	{
		setPret(getProdus(getAll(memory), poz), pret);
		return 1;
	}
	return 0;
...
int updateCantitate(Repo* memory, Produs* prod, int cantitate)
{
	...
	*/
	...
	if (poz != -1)
	{
		setCantitate(getProdus(getAll(memory), poz), cantitate);
		return 1;
	}
	return 0;
}
