...
Vector* sortareCantitate(Service * serv, int reverse)
{
	...
	*/
	...
	for (i = 0; i < getDim(iaTot(serv)) - 1; i++)
	{
		...
		{
			if ((reverse == 0) && (getCantitate(getProdus(iaTot(serv), j)) < getCantitate(getProdus(iaTot(serv), i))))
			{
				swapElem(iaTot(serv), i, j);
			}
			else {
				if ((reverse == 1) && (getCantitate(getProdus(iaTot(serv), j)) > getCantitate(getProdus(iaTot(serv), i))))
				{
					swapElem(iaTot(serv), i, j);
				}
			}
...
Vector* sortarePret(Service  * serv, int reverse)
{
	...
	*/
	...
	for (i = 0; i < getDim(iaTot(serv)) - 1; i++)
	{
		for (j = i + 1; j < getDim(iaTot(serv)); j++)
		{
			if ((reverse == 0) && (getPret(getProdus(iaTot(serv), j)) < getPret(getProdus(iaTot(serv), i))))
			{
				swapElem(iaTot(serv), i, j);
			}
			else {
				if ((reverse == 1) && (getPret(getProdus(iaTot(serv), j)) > getPret(getProdus(iaTot(serv), i))))
				{
					swapElem(iaTot(serv), i, j);
				}
			}
...
void getProducatorList(Vector * list, Service * serv, char *producator)
{
	...
	*/
	...
	{
		if (strcmp(getProducator(getProdus(iaTot(serv), i)), producator) == 0)
		{
			append(list, getProdus(iaTot(serv), i));
		}
	}
}
...
void getPretList(Vector* list, Service * serv, double pret)
{
	...
	*/
	...
	{
		if (getPret(getProdus(iaTot(serv), i)) == pret)
		{
			append(list, getProdus(iaTot(serv), i));
		}
	}
...
void getCantitateList(Vector* list, Service * serv, int cantitate)
{
	...
	*/
	for (int i = 0; i < getDim(iaTot(serv)); i++)
	{
		if (getCantitate(getProdus(iaTot(serv), i)) == cantitate)
		{
			append(list, getProdus(iaTot(serv), i));
		}
	}
...
void test_iaTot()
{
	...
	assert(comparaProduse(getProdus(iaTot(serv), 0), prod1) == 0);
	assert(comparaProduse(getProdus(iaTot(serv), 1), prod2) == 0);
	assert(comparaProduse(getProdus(iaTot(serv), 2), prod3) == 0);
	distrugeService(serv);
}
...
void test_sortarePret()
{
	...
	assert(getId(getProdus(listaSortata(serv1, '1'), 0)) == 101);
	assert(getId(getProdus(listaSortata(serv1, '1'), 1)) == 103);
	assert(getId(getProdus(listaSortata(serv1, '1'), 2)) == 102);
	...
	assert(getId(getProdus(listaSortata(serv2, '2'), 0)) == 103);
	assert(getId(getProdus(listaSortata(serv2, '2'), 1)) == 102);
	assert(getId(getProdus(listaSortata(serv2, '2'), 2)) == 101);
...
void test_sortareCantitate()
{
	...
	assert(getId(getProdus(listaSortata(serv1, '3'), 0)) == 103);
	assert(getId(getProdus(listaSortata(serv1, '3'), 1)) == 102);
	assert(getId(getProdus(listaSortata(serv1, '3'), 2)) == 101);
	...
	assert(getId(getProdus(listaSortata(serv2, '4'), 0)) == 101);
	assert(getId(getProdus(listaSortata(serv2, '4'), 1)) == 103);
	assert(getId(getProdus(listaSortata(serv2, '4'), 2)) == 102);
