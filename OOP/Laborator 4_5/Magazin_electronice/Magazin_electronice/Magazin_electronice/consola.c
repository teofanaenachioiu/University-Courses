	...
	*/
	...
	if (getDim(lista) > 0)
	{
		...
		for (i = 0; i < getDim(lista); i++)
		{
			...
			printf("|  Produs: %d | %s | %s | %s | %.2f | %d\n", getId(get(lista, i)), getTip(get(lista, i)), getModel(get(lista, i)), getProducator(get(lista, i)), getPret(get(lista, i)), getCantitate(get(lista, i)));
	...
	*/
	...
	if (getDim(listaSortata(serv, mod)) > 0)
	{
		...
		for (i = 0; i < getDim(listaSortata(serv, mod)); i++)
		{
			...
			printf("|  Produs: %d | %s | %s | %s | %.1f | %d\n", getId(get(listaSortata(serv, mod), i)), getTip(get(listaSortata(serv, mod), i)), getModel(get(listaSortata(serv, mod), i)), getProducator(get(listaSortata(serv, mod), i)), getPret(get(listaSortata(serv, mod), i)), getCantitate(get(listaSortata(serv, mod), i)));
		}
	}
	else printf("|  Nu exista produse in stoc.\n");
}
...
//functie incearca sa faca undo, daca reuseste returneaza 0 altfel 1
void makeUndoC(Consola * cons) {
	if (makeUndo(cons->s) != 0)
		printf("nu mai mere undo cumetre\n");
	else
