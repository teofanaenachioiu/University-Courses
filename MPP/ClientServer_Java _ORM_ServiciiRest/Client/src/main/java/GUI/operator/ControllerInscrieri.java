...
public class ControllerInscrieri extends UnicastRemoteObject implements Initializable, IObserver {
    @FXML
    TextField fieldNume;
    @FXML
    ...
    public ControllerInscrieri() throws RemoteException {
    }

    void setData(IServer server, User user){
        this.user=user;
        this.server=server;
    ...
    public void handleMarcat(){
        List<Proba> probe11=checkComboBox.getCheckModel().getCheckedItems();
        Proba[] probe = new Proba[probe11.size()];
        for(int i=0;i< probe11.size();i++){
            probe[i]=probe11.get(i);
        }
        ...
        try{
            server.inscriereParticipant(nume,varsta,probe,this.user.getID());
