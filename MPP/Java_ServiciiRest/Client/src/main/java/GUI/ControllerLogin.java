...
import GUI.admin.ControllerAdmin;
...
public class ControllerLogin extends UnicastRemoteObject implements IObserver {
    ...
    private void initViewAdmin(User user) throws IOException {
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("/ViewAdmin.fxml"));
        AnchorPane adminLayout = loader.load();
        Scene sceneAdmin = new Scene(adminLayout, 600, 400);

        this.controllerAdmin = loader.getController();

        controllerAdmin.setLoginScene(loginScene);
        controllerAdmin.setPrimaryStage(primaryStage);
        controllerAdmin.setData(server, user);

        this.adminScene = sceneAdmin;
    }
